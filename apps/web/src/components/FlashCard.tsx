...
export function FlashCard({ card }: { card: Card }) {
  return (
    <motion.article
      className="flex h-full w-full flex-col overflow-y-auto rounded-3xl border border-slate-200 bg-white p-6 shadow-xl dark:border-slate-800 dark:bg-slate-900 sm:p-8"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    >
      <div className="mb-3 flex items-center gap-2">
        ...
        <span className="text-xs text-slate-400">~{card.readingSeconds}s read</span>
      ...
      <p className="mt-3 text-slate-700 dark:text-slate-300">{card.explanation}</p>
      ...
      {card.analogy && (
        <Section title="Analogy">
          <p className="text-sm italic text-slate-600 dark:text-slate-400">{card.analogy}</p>
        </Section>
      )}
      ...
      {card.tags.length > 0 && (
        <div className="mt-auto flex flex-wrap gap-1.5 pt-5">
          {card.tags.map((t) => (
            <span key={t} className="rounded-md bg-slate-100 px-2 py-0.5 text-xs text-slate-500 dark:bg-slate-800">
              #{t}
            </span>
          ))}
        </div>
